...
class MoreController extends PublicController{
	...
	public function fankui(){
		...
		$where="1=1";
		$message!='' ? $where.=" AND content like '%$message%'" : null;
		//=========================
		//define  每页显示的数量
		//=========================
		define('rows',20);
		...
		foreach ($fankui as $k => $v) {
			$fankui[$k]['uname'] = M('user')->where('id='.intval($v['uid']))->getField('name');
		}
		//=============
		//将变量输出
		//=============
	...
	public function delfk(){
		...
		$res = M('fankui')->where('id='.intval($did))->delete();
		if ($res) {
			$this->success('操作成功！');
			exit();
		}else {
